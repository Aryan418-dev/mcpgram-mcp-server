...
import {
  ...
  listUserWorkspaces,
} from "../../../../src/oauth/supabase";
import { publicBaseUrl } from "../../../../src/oauth/config";
import type { WorkspaceGrant } from "../../../../src/oauth/types";
  ...
  try {
    ...
    const requestedIds = [
      ...new Set(
        [
          ...(body.workspace_ids ?? []),
          ...(body.workspace_id ? [body.workspace_id] : []),
        ].filter(Boolean)
      ),
    ];
    if (requestedIds.length === 0) {
      return Response.json(
        { error: "Select at least one workspace (workspace_ids)" },
        { status: 400 }
      );
    }

    const allowed = await listUserWorkspaces(userData.user.id);
    const allowedMap = new Map(allowed.map((w) => [w.id, w.name]));
    const missing = requestedIds.filter((id) => !allowedMap.has(id));
    if (missing.length > 0) {
      return Response.json(
        { error: `Workspace not accessible: ${missing.join(", ")}` },
        { status: 403 }
      );
    }

    const labelBase = `Claude OAuth (${userData.user.email ?? userData.user.id.slice(0, 8)})`;
    const workspaces: WorkspaceGrant[] = [];
    for (const id of requestedIds) {
      const api_key = await issueWorkspaceApiKey(id, labelBase);
      workspaces.push({ id, name: allowedMap.get(id), api_key });
    }
