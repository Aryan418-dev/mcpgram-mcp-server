...
function McpgramLogoMark() {
  ...
  const src = MCPGRAM_LOGO_CANDIDATES[Math.min(srcIndex, MCPGRAM_LOGO_CANDIDATES.length - 1)];
  return (
    <img
      src={src}
      alt="MCPGRAM"
      ...
      onError={() => {
        setSrcIndex((i) => (i + 1 < MCPGRAM_LOGO_CANDIDATES.length ? i + 1 : i));
      }}
    />
  );
}
