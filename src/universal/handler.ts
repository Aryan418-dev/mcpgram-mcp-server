    ...
    case "discover_tools": {
      ...
      try {
        ...
        const tools = Array.isArray((result as { tools?: unknown }).tools)
          ...
          : [];
        return textResult({
          ok: Boolean((result as { connection_ok?: boolean }).connection_ok ?? true),
          server_id: serverId,
          status: (result as { status?: string }).status ?? null,
          tool_count: Number((result as { tool_count?: number }).tool_count ?? tools.length),
          tools,
          message:
            (result as { message?: string }).message ?? `Refreshed ${tools.length} tools`,
          action: name,
        });
      } catch (err) {
        if (err instanceof ApiError) {
